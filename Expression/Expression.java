...
public class Expression {
	...
	public static void makeVariableLists(String expr, ArrayList<Variable> vars, ArrayList<Array> arrays) {
		...
		if (Pattern.matches("[a-zA-Z]+", token.get(token.size() - 1))
				&& isArray(arrays, token.get(token.size() - 1)) == -1) {
	...
	private static boolean OOO(String current, String top) {// order of operations
	...
	public static float evaluate(String expr, ArrayList<Variable> vars, ArrayList<Array> arrays) {
		...
		while (str.hasMoreTokens()) {// reads through expression
			...
			if (current.equals(" ") || current.equals("\t")) {
				...
				continue;
			} else if (isVariable(vars, current) != -1) {// adds the variable value to stack
